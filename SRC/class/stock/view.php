...
?>
	...
	<form name="form" id="form" action="index.php" method="post">
		...
		<table border="0" cellpadding="5" cellspacing="1">
			<tr>
				<th>除外</th>
				<td><input type="radio" name="del" value="1" <?php if ($param["sDel"] === "1" || $param["sDel"] == "") print ' checked="checked"' ?> /> 非除外
			...
			<tr>
				...
				<td>
					...
					for ($i = 0; $i < 5; $i++) {
					?>
						<input type="radio" name="rank" value="<?php print $i + 1; ?>" <?php if (($i == 0 && $param["rank"] == "") || $param["rank"] == $i + 1) print ' checked="checked"'; ?> /> <?php print fnRankName($i); ?>
					<?php
					}
					?>
				</td>
			...
			<tr>
				...
				<td>
					...
					for ($i = 0; $i < 4; $i++) {
					?>
						<input type="radio" name="distance" value="<?php print $i + 1; ?>" <?php if (($i == 0 && $param["distance"] == "") || $param["distance"] == $i + 1) print ' checked="checked"'; ?> /> <?php print fnDistanceName($i); ?>
					<?php
					}
					?>
				</td>
